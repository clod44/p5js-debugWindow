class DebugWindow {
    ...
    add(label, valueRef, isGraph = false, code = "default", order = 0) {
        if (isGraph) {
            let graph = new DebugWindowGraph(label);
            graph.resize(this.size.x - this.padding * 2, 50);
            this.values.push({ label, valueRef, isGraph, graph, code, order });
        } else {
            this.values.push({ label, valueRef, isGraph, code, order });
        }
    ...
    remove(codeToRemove) {
        this.values = this.values.filter(item => item.code !== codeToRemove);
    }
...
class DebugWindowGraph {
    ...
    update(val) {
        ...
        this.g.text(this.title, 0, 0);
        this.g.textAlign(RIGHT, TOP);
        this.g.text("" + (this.maxValue.toFixed(2)), this.size.x, 0);
        this.g.textAlign(RIGHT, CENTER);
        this.g.text("" + (this.valueHistory[this.valueHistory.length - 1].toFixed(2)), this.size.x, this.size.y * 0.5);
